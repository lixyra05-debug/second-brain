...
## Stack
- **Python 3** en venv (commun à tous les timers : `notebooklm-veillebot/venv`), **systemd user** (14 unités actives), SQLite (`state.db`, `kanban.db`), Telegram (gateway + alertes).
...
- **Unités** : `~/.config/systemd/user/*.{timer,service}` (14 actives + services de test)
